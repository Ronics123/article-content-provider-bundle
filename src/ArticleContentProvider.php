...
class ArticleContentProvider
{
    ...
    private bool $isBold;
    private WordDecoratorInterface $workDecorator;
    private ?EventDispatcherInterface $dispatcher;

    public function __construct(WordDecoratorInterface $workDecorator, EventDispatcherInterface $dispatcher = null, bool $isBold = true)
    {
        $this->isBold = $isBold;
        $this->workDecorator = $workDecorator;
    ...
    private function paste(string $text, string $word, int $wordsCount = 1): string
    {
        ...
        for ($i = 0; $i < $wordsCount; $i++) {
            ...
            array_splice($words, $position, 0, $this->workDecorator->decorator($event->getWord(), $this->isBold));
        }
